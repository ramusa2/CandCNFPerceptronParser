...
public class LogProbabilityContextExtractor extends ContextExtractor<MultitaggerExample> implements Externalizable {

	private int dimension;
	
	private DoubleMatrix output;

	public LogProbabilityContextExtractor(int numCatsToLookAt) {
	...
	public DoubleMatrix extract(MultitaggerExample context) {
		for(int c=0; c<this.output.length; c++) {
			this.output.put(c, Math.log(context.getItem().getProb(c) + 1.0));
		}
		return this.output;
	}
