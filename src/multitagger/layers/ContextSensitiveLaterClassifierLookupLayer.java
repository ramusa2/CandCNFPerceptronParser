...
public class ContextSensitiveLaterClassifierLookupLayer extends
		EmbeddingMultitaggerInputLayer {
	...
	public ContextSensitiveLaterClassifierLookupLayer(int numCatsToLookAt, int contextWindowSize, 
		...
		for(int c=-this.CXT_WINDOW_SIZE; c<=this.CXT_WINDOW_SIZE; c++) {
			...
			this.addInputSegment(categoryEmbeddingDimension);
			// TODO: add fields for surrounding words
			//this.addInputSegment(wordEmbeddingDimension);
		}
	}
...
*/
	...
	protected void fillOutputVector(DoubleMatrix output,
		...
		for(int k=0; k<this.NUM_CATS_IN_LIST; k++) {
			...
			//this.setSegmentOutput(output, categoryEmbedding, thisItem.getProb(k), segment++);
		}
