...
public class MappingFunctionInputLayer extends EmbeddingMultitaggerInputLayer {
	...
	public MappingFunctionInputLayer(int numCatsToLookAt, int contextWindowSize, 
	...
	public void saveWeightsToFile(File saveDir) {
		...
		try {
		PrintWriter pw = new PrintWriter(new File("mapping_dimensions"));
		for(int l=0; l<this.mappingFunctions.size(); l++) {
			LinearLayer mappingLayer = this.mappingFunctions.get(l);
			pw.println(this.mappingDimensions.get(l)+" "+mappingLayer.getNumberOfNodes());
			mappingLayer.saveWeightsToFile(new File(saveDir.getPath()+File.separator+"mapping_layer_"+l));
		}
		pw.close();
		} catch(Exception e) {
			System.out.println("Failed to save context-mapping input layer to disk.");
		}
	}
		...
		try {
			...
			while(sc.hasNextLine()) {
				...
				if(!line.isEmpty()) {
					...
					int inDim = Integer.parseInt(toks[0]);
					int outDim = Integer.parseInt(toks[1]);
					LinearLayer mappingFunction = new LinearLayer(inDim, outDim);
					mappingFunction.loadWeightsFromFile(new File(loadDir.getPath()+File.separator+"mapping_layer_"+l));
					this.mappingFunctions.add(mappingFunction);
					this.mappingDimensions.add(inDim);
					l++;
				}
			}
			sc.close();
		} catch(Exception e) {
			System.out.println("Failed to load context-mapping input layer from disk.");
		}
	}
