...
class NF {
   ...
   * @return boolean
   */
  static boolean unaryNF(Rule_Type History, Rule_Type type) {
    ...
    case Full:
      ...
      // type-raised.
      if (History.equals(Rule_Type.FW_CONJOIN)
          || History.equals(Rule_Type.BW_CONJOIN)) {
        if (type.equals(Rule_Type.FW_TYPERAISE)
            || type.equals(Rule_Type.BW_TYPERAISE)) {
          return false;
        }
      }
      return true;
   ...
   * @return boolean
   */
  static boolean binaryNF(Rule_Type A, int Aa,
      Rule_Type B, int Ba, Rule_Type C, int Ca) {
    if(B == null || C == null) {
      return true;
    }
    switch (Configuration.NF) {
    case Eisner:
      return Eisner_NF(A, Aa, B, Ba, C, Ca);
    case Eisner_Orig:
      return Eisner_Orig_NF(A, B, C);
    case Full_noPunct:
      return Full_noP_NF(A, Aa, B, Ba, C, Ca);
    case Full:
      return Full_NF(A, Aa, B, Ba, C, Ca);
    case None:
      return true;
    }
    return false;
  ...
  private static boolean Eisner_NF(Rule_Type A, int Aa,
      Rule_Type B, int Ba, Rule_Type C, int Ca) {
  ...
  private static boolean Full_noP_NF(Rule_Type A, int Aa,
      Rule_Type B, int Ba, Rule_Type C, int Ca) {
    if (B.equals(Rule_Type.LEX) || C.equals(Rule_Type.LEX)) {
      return false;
    }
    ...
    case BW_CONJOIN_TR:
    case BW_CONJOIN:
      // If A is a Conjoin then can't have both either B or C is not a TYPERAISE
      // A BW_CONJOIN --> B not FW|BW_TYPERAISE || C not FW|BW_TYPERAISE
      if ((B.equals(Rule_Type.FW_TYPERAISE) || B.equals(Rule_Type.BW_TYPERAISE))
          && C.equals(Rule_Type.FW_CONJOIN_TR)) {
        return false;
      }
  ...
  private static boolean Full_NF(Rule_Type A, int Aa,
      Rule_Type B, int Ba, Rule_Type C, int Ca) {
    if (B.equals(Rule_Type.LEX) || C.equals(Rule_Type.LEX)) {
      return false;
    }
    ...
    case FW_PUNCT:          // NOTE:  Not in implementation of the paper
      if (B.equals(Rule_Type.FW_TYPERAISE)
          || C.equals(Rule_Type.BW_TYPERAISE)) {
        return false;
      }
      return Full_noP_NF(A, Aa, B, Ba, C, Ca);
    case BW_PUNCT:
  ...
  static boolean PunctNF(Rule_Type A, Rule_Type B, Punctuation Bp,
      Rule_Type C, Punctuation Cp){
    if(A == Rule_Type.FW_PUNCT) {          // NOTE:  Not in implementation of the paper
      if (B.equals(Rule_Type.FW_TYPERAISE)
          || C.equals(Rule_Type.BW_TYPERAISE)) {
        return false;
      }
    } else if (A == Rule_Type.BW_PUNCT) {
      // Must form constituent first
      if (B.equals(Rule_Type.FW_TYPERAISE)
          || C.equals(Rule_Type.BW_TYPERAISE)) {
        return false;
